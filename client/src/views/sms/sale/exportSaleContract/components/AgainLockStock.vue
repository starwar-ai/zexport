...
defineOptions({ name: 'AgainLockStock' })
const dialogTableVisible = ref(false)
const message = useMessage()
const tableData = ref([])
const index = ref(0)
const realLockQuantity = ref(0)
const params = ref({})
...
const open = async (data) => {
  realLockQuantity.value = data.realLockQuantity
  params.value = data
...
const handleSure = async () => {
  ...
  // if (lockQuantityTotal > realLockQuantity.value) {
  //   message.warning(`锁定数量必须等于原锁定数量${realLockQuantity.value}，请重新输入锁定数量！`)
  //   return false
  // }
  await ExportSaleContractApi.reLock({
    saleContractCode: params.value.saleContractCode,
    itemList: [
      {
        saleContractItemId: params.value.saleContractItemId,
        lockSaveReqVOList: list
      }
    ]
  })
  // emit('success')
