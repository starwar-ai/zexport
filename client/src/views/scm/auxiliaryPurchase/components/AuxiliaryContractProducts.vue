...
import EplusMoney from '@/components/EplusMoney/src/EplusMoney.vue'
...
let tableColumns = reactive<TableColumn[]>([
      ...
      return (
        <span>
          ...
          <el-button
            ...
            disabled={
              row?.auxiliarySkuType === 2 || !row?.auxiliarySkuType || props?.type === 'contract'
            }
          >
      ...
      return (
        <span>
          ...
          <el-button
            ...
            disabled={
              row?.auxiliarySkuType === 1 || !row?.auxiliarySkuType || props?.type === 'contract'
            }
          >
      ...
      return (
        <span>
          <el-select
            ...
            disabled={!row.auxiliaryPurchaseContractCode || props?.type === 'contract'}
            onChange={(val) => {
              if (val) {
  ...
  },
  {
    ...
    slot: (item, row: Recordable, index: number) => {
      return (
        <EplusMoney
          v-model:amount={row.withTaxPrice.amount}
          v-model:currency={row.withTaxPrice.currency}
