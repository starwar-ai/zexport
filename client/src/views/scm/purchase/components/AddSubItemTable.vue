...
let tableColumns = reactive<TableColumn[]>([
  {
    ...
    slot: (item, row: Recordable, index: number) => {
      return (
        <div>
          <span>{row.linkContractCode || '-'}</span>
          <el-button
            class="ml5px"
            onClick={() => {
...
const handleSure = (data) => {
  tableList.value[data.index].linkContractCode = data.code
}
...
const handleAdd = async () => {
  const newRecord = {
    ...
    calculationType: 1,
    contractCode: props.formData.code,
    linkContractCode: ''
  }
...
watch(
  () => tableList.value,
  (list) => {
    let amountSum = list.map((item: any) => {
