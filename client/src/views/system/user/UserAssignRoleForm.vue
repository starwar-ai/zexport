...
const open = async (row: UserApi.UserVO) => {
  ...
  try {
    ...
    let roleIdList = await PermissionApi.getUserRoleList(row.id)
    // let res = roleList.value.map((item) => {
      ...
      roleIdList.map((role) => {
        if (item.id === role.roleId) {
          sameObjects.push({
            id: item.id,
            name: item.name,
            defaultFlag: role.defaultFlag
          })
        }
      })
    })

    formData.value.roleIds = sameObjects.filter((item) => item)
  } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    formData.value.roleIds.forEach((item) => {
      item.roleId = item?.id
    })
