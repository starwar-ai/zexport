          ...
          <el-row>
            <el-col :span="16">
              <div class="checkbox-header">
                <div class="checkbox-header-title"> 基础信息 </div>
                <el-button
                  round
                  size="small"
                  class="no-border-button"
                  @click="checkedAllClick(`parent`)"
                >
                  {{ checkAllParentLabel }}
                </el-button>
              </div>
              <el-checkbox-group v-model="checkedParentColumnLabels">
                <el-row>
                  <el-col
                    v-for="col in parentColumnSettings"
                    :key="col.prop"
                    :span="8"
                  >
                    <el-checkbox
            ...
            <el-col
              ...
              class="bl1 pl10px"
            >
              <column-config-table mode="parent" />
            </el-col>
          </el-row>
        </el-tab-pane>
        <el-tab-pane
        ...
        >
          <el-row>
            <el-col :span="16">
              ...
              <el-checkbox-group v-model="checkedColumnLabels">
                <el-row>
                  <el-col
...
import { getPreferences, updateSetPreferences } from '@/api/system/user/index'
...
//查询当前页面配置项
let params = `${pageKey}&tabIndex=${props.tabIndex}`
getPreferences(params).then((res) => {
  if (res?.children) {
    ...
    columnSettings.forEach((item) => {
      if (selectedCols.includes(item.prop)) {
        // 勾选的column
        selectColumn.push(item)
      }
    })
    // 按照列设置排好的顺序进行对比，让表格中列的排序和列设置中的排列一致
    let sortSelectColumn = []
    selectedCols.forEach((element) => {
      selectColumn.forEach((obj) => {
        if (obj.prop === element) {
          sortSelectColumn.push(obj.label)
        }
      })
    })
    checkedColumnLabels.value = sortSelectColumn
  }
  if (res?.parent) {
    let parentSelectedCols = res.parent.map((item) => item.prop)
    let selectParentColumn = []
    parentColumnSettings.forEach((item) => {
      if (parentSelectedCols.includes(item.prop)) {
        selectParentColumn.push(item)
      }
    })
    // 按照父列设置中排好的顺序进行对比
    let sortSelectParentColumn = []
    parentSelectedCols.forEach((item) => {
      selectParentColumn.forEach((obj) => {
        if (obj.prop === item) {
          sortSelectParentColumn.push(obj.label)
        }
      })
    })
    checkedParentColumnLabels.value = sortSelectParentColumn
  }
  handleSave(false)
})
...
const handleSave = async (flag = true) => {
  ...
  if (flag) {
    await updateSetPreferences({
      setPreference: {
        tabIndex: props.tabIndex,
        pageKey,
        parent: orderedParentOrderColumnSettings,
        children: orderedColumnOrderSettings
      }
    })
  }
}
...
const checkedAllClick = (value) => {
  ...
  if (value === 'parent') {
    //--父全选
    //  if (checkedParentColumnLabels.value.length < checkedDefaultParentColumnLabels.length) {
    if (checkedParentColumnLabels.value.length < parentColumnSettings.length) {
      checkAllParentLabel.value = '取消全选'
      checkeLabels = parentColumnSettings.map((it) => it.label)
    } else {
      checkAllParentLabel.value = '全选'
      checkeLabels = parentColumnSettings
        .filter((item) => {
          return item.disable
        })
        .map((it) => it.label)
    }
    checkedParentColumnLabels.value = checkeLabels
  } else {
    //--子全选
    //  if (checkedColumnLabels.value.length < checkedDefaultColumnLabels.length) {
    if (checkedColumnLabels.value.length < columnSettings.length) {
...
watch(
  ...
  (value) => {
    ...
    if (value.length === columnSettings.length) {
      checkAllLabel.value = '取消全选'
    } else {
      checkAllLabel.value = '全选'
    }
...
watch(
  ...
  (value) => {
    ...
    if (value.length === parentColumnSettings.length) {
      checkAllParentLabel.value = '取消全选'
    } else {
      checkAllParentLabel.value = '全选'
    }
