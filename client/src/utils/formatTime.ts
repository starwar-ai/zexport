...
const isTimestamp = (value) => {
  return dayjs.unix(Number(value)).isValid()
}
...
export function formatPast(param: string | Date, format = 'YYYY-mm-dd HH:MM:SS'): string {
  ...
  // 是否是对象
  if (typeof param === 'string' || typeof param === 'object') t = new Date(param).getTime()
  else t = param
  // 当前时间戳 - 传入时间戳
  ...
  if (time < 10000) {
    ...
    // 超过3天
    let date: Date
    if (typeof param === 'string' || typeof param === 'object') date = new Date(param)
    else date = param
