...
public interface OperateLogConvert {
    ...
    default List<OperateLogRespDTO> convertOperateLogRespDTO(List<OperateLogDO> list, Map<Long, AdminUserDO> userMap) {
        return CollectionUtils.convertList(list, log -> {
            OperateLogRespDTO logVO = BeanUtils.toBean(log, OperateLogRespDTO.class);
            MapUtils.findAndThen(userMap, log.getUserId(), user -> logVO.setUserNickName(user.getNickname()));
            return logVO;
        });
    }
