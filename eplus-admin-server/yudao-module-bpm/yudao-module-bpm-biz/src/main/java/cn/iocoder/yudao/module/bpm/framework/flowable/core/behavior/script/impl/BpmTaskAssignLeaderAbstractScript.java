...
import static cn.iocoder.yudao.module.bpm.enums.ErrorCodeConstants.START_USER_DEPT_NOT_EXISTS;
...
public abstract class BpmTaskAssignLeaderAbstractScript implements BpmTaskAssignScript {
    ...
    protected Set<Long> calculateTaskCandidateUsers(DelegateExecution execution, int level) {
        ...
        for (int i = 0; i < level; i++) {
            ...
            if (dept == null) {
                ...
                if (dept == null) { // 找不到发起人的部门，所以无法使用该规则
                    throw exception(START_USER_DEPT_NOT_EXISTS, startUserId);
                }
            } else {
        ...
        Set<Long> leaderUserIds = dept.getLeaderUserIds();
        if (CollUtil.isNotEmpty(leaderUserIds)){
            return leaderUserIds;
        }
        // 如果当前审批节点部门领导为空则向上寻找
        return deptApi.getParentLeaderUserIds(dept.getId());
        // 如果父节点部门领导均为空则自动通过当前流程实例
//        if (CollUtil.isEmpty(leaderUserIds)){
//            bpmProcessInstanceService.setAutoApproveInstanceCache(execution.getProcessInstanceId());
//            return Set.of();
//        }
    }
