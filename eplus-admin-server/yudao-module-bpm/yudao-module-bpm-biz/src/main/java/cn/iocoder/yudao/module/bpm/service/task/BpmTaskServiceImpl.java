...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    private static final ThreadLocal<List<String>> AUTO_APPROVE = new TransmittableThreadLocal<>();
    ...
    public Task approveTask(Long userId, @Valid BpmTaskApproveReqVO reqVO) {
        // 1.1 校验任务存在
        ...
        // 3.1 更新 task 状态、原因
        updateTaskStatusAndReason(task.getId(), BpmProcessInstanceResultEnum.APPROVE.getResult(), reqVO.getReason());
        // 完成任务，审批通过
        synchronized (task.getId().intern()) {
            Task existingTask = taskService.createTaskQuery().taskId(task.getId()).singleResult();
...
//        taskService.complete(task.getId(), instance.getProcessVariables());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Task rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        Task task = validateTask(userId, reqVO.getId());
        // 校验流程实例存在
        ProcessInstance instance = processInstanceService.getProcessInstance(task.getProcessInstanceId());
        if (instance == null) {
    ...
    public void createTaskExt(Task task) {
        ...
        // 自动通过
        List<String> taskIds = AUTO_APPROVE.get();
        if (CollUtil.isNotEmpty(taskIds) && taskIds.contains(task.getId())) {
            taskExtDO.setResult(BpmProcessInstanceResultEnum.APPROVE.getResult());
            List<String> result = taskIds.stream().filter(s -> !s.equals(task.getId())).toList();
            AUTO_APPROVE.set(result);
        }else {
    ...
    @Override
    public BpmTaskExtDO getTaskExtListByBpmProcessInstanceId(String processInstanceId) {
        return taskExtMapper.selectOne(new LambdaQueryWrapperX<BpmTaskExtDO>().eq(BpmTaskExtDO::getTaskId, processInstanceId).orderByDesc(BpmTaskExtDO::getUpdateTime));
    }

    @Override
    public BpmTaskExtDO getTaskExtByInstanceId(String processInstanceId) {
        return taskExtMapper.selectOne(new LambdaQueryWrapperX<BpmTaskExtDO>().eq(BpmTaskExtDO::getProcessInstanceId, processInstanceId).orderByDesc(BpmTaskExtDO::getUpdateTime));
    }
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        if (task.getAssignee().equals(reqVO.getAssigneeUserId().toString())) { // 校验当前审批人和被转派人是同一人
        ...
        // 2. 添加委托意见
        AdminUserRespDTO currentUser = adminUserApi.getUser(userId);
        taskService.addComment(taskId, task.getProcessInstanceId(), BpmCommentTypeEnum.TRANSFER.transformStr(),
    ...
    public void dealSimpleTask(String taskId, String processInstanceId, Long assigneeId) {
        List<BpmTaskExtDO> bpmTaskExtDOList = taskExtMapper.selectList(new LambdaQueryWrapperX<BpmTaskExtDO>().eq(BpmTaskExtDO::getProcessInstanceId, processInstanceId).eq(BpmTaskExtDO::getResult, BpmProcessInstanceResultEnum.APPROVE.getResult()).orderByDesc(BpmTaskExtDO::getCreateTime));
        if (CollUtil.isEmpty(bpmTaskExtDOList)) {
            return;
        }
        Optional<BpmTaskExtDO> lastNodeOpt = bpmTaskExtDOList.stream().filter(s -> BpmProcessInstanceResultEnum.APPROVE.getResult().equals(s.getResult())).sorted(Comparator.comparing(BpmTaskExtDO::getCreateTime).reversed()).findFirst();
        if (lastNodeOpt.isPresent()) {
            BpmTaskExtDO lastNode = lastNodeOpt.get();
            if (assigneeId.equals(lastNode.getAssigneeUserId())) {
                ProcessInstance processInstance = processInstanceService.getProcessInstance(processInstanceId);
                validateTask(WebFrameworkUtils.getLoginUserId(), taskId);
                // 完成任务，审批通过
                taskService.complete(taskId, processInstance.getProcessVariables());
                // 由于先完成了任务，所以需要设置一个标识，让流程引擎自动审批通过
                List<String> stringList = AUTO_APPROVE.get();
                if (CollUtil.isEmpty(stringList)) {
                    AUTO_APPROVE.set(Collections.singletonList(taskId));
                }
            }
        }
    ...
    public List<Long> getApproveAssigneeUserIdList(String processInstanceId) {
        List<BpmTaskExtDO> bpmTaskExtDOList = taskExtMapper.selectList(BpmTaskExtDO::getProcessInstanceId, processInstanceId, BpmTaskExtDO::getResult, BpmProcessInstanceResultEnum.APPROVE.getResult());
        if (CollUtil.isEmpty(bpmTaskExtDOList)){
            return List.of();
        }
        return bpmTaskExtDOList.stream().map(BpmTaskExtDO::getAssigneeUserId).toList();
    }

    @Override
    public void autoComplete(String taskId, String processInstanceId,String definitionName, Long assigneeUserId) {
        runtimeService.setProcessInstanceName(processInstanceId, definitionName);
        ProcessInstance processInstance = processInstanceService.getProcessInstance(processInstanceId);
        validateTask(WebFrameworkUtils.getLoginUserId(), taskId);
        taskService.complete(taskId, processInstance.getProcessVariables());
        // 由于先完成了任务，所以需要设置一个标识，让流程引擎自动审批通过
