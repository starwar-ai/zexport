...
public class ConfigController {
    ...
    public CommonResult<List<ConfigRespVO>> getConfigPage() {
        List<ConfigRespVO> configList = configService.getConfigList();
    ...
    @GetMapping("/get-invoice-rate")
    @Operation(summary = "获取发票税率")
    @Parameter(name = "isAbroad",description = "是否国外")
    public CommonResult<List<ConfigMapVO>> getInvoiceRate(@Valid Integer isAbroad) {
        return success(configService.getInvoiceRate(isAbroad));
    }
    @GetMapping("/get-json")
    @Operation(summary = "获取json类型数据")
    @Parameter(name = "configType",description = "配置表参数键名")
    @Parameter(name = "type",description = "数据类型")
    public CommonResult<List<ConfigJsonRespVO>> getJsonList(@Valid String configType,@Valid String type) {
        return success(configService.getJsonList(configType,type));
    }
