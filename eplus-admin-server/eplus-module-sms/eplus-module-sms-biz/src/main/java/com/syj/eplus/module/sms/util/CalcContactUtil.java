...
public class CalcContactUtil {
     ...
     */
    public static void checkItemCost(SaleContractSaveReqVO saleContract, Map<String, String> configMap, Map<String, BigDecimal> currencyRateMap, String currency) {
...
//        saleContract.setEstimatedTotalFreight(new JsonAmount().setAmount(totalCabinetFee).setCurrency(CalculationDict.CURRENCY_RMB));
        // 计算明细费用
        checkItemFee(saleContract,saleContractItemList, totalCabinetFee, totalVolume, trailerFee, currencyRateMap,currency,configMap);
        // 佣金金额
...
//        saleContract.setAccessoriesPurchaseTotal(accessoriesPurchaseTotal);
     ...
     */
    private static void checkItemFee(SaleContractSaveReqVO saleContract,List<SaleContractItem> saleContractItemList, BigDecimal estimatedTotalFreight,
                                     BigDecimal totalVolume, JsonAmount trailerFee, Map<String, BigDecimal> currencyRateMap,String currency,Map<String, String> configMap) {
        // 表单拖柜费/体积合计
        ...
        saleContractItemList.forEach(saleContractItem -> {
            ...
            BigDecimal totalSaleAmount = NumUtil.mul(saleContractItem.getQuantity(), saleContractItem.getUnitPrice().getAmount()).setScale(CalculationDict.DECIMAL_PRECISION, RoundingMode.HALF_UP);

            //成本
            saleContractItem.setTotalSaleAmount(new JsonAmount().setAmount(totalSaleAmount).setCurrency(saleContractItem.getUnitPrice().getCurrency()));
            AtomicReference<BigDecimal> addAmountRmb = new AtomicReference<>(NumUtil.mul(totalSaleAmount, currencyRateMap.get(currency)));
            ...
            //采购
            Optional.ofNullable(saleContractItem.getPurchaseWithTaxPrice()).ifPresent(s->{
                // 采购币种
                String purhcaseCurrency = Objects.isNull(s.getCurrency())?CalculationDict.CURRENCY_RMB:s.getCurrency();
                // 采购币种汇率
                BigDecimal purchaseRate = currencyRateMap.get(purhcaseCurrency);
                Optional.ofNullable(purchaseRate).ifPresent(rate->{
                    // 采购总金额
                    BigDecimal purchaseTotalAmount = NumUtil.mul(NumUtil.mul(s.getAmount(), rate), saleContractItem.getNeedPurQuantity());
            ...
            //包材采购成本  -- 采购单价 * 销售数量
            Optional.ofNullable(saleContractItem.getPurchasePackagingPrice()).ifPresent(s->{
                // 包材采购币种
                String packageCurrency = Objects.isNull(s.getCurrency())?CalculationDict.CURRENCY_RMB:s.getCurrency();
                // 采购币种汇率
                BigDecimal purchaseRate = currencyRateMap.get(packageCurrency);
                Optional.ofNullable(purchaseRate).ifPresent(rate->{
                    // 采购总金额
                    BigDecimal packingPurchaseTotalAmount = NumUtil.mul(NumUtil.mul(s.getAmount(), rate), saleContractItem.getNeedPurQuantity());
                    subAmountRmb.set(NumUtil.add(subAmountRmb.get(), packingPurchaseTotalAmount));
                });
            });
            // 预估总费用 -- (表单预估总运费 * 外箱体积 * 箱数) / 总体积
            JsonAmount forecastTotalCostJson = BeanUtil.copyProperties(itemEstimatedTotalFreight,JsonAmount.class);
            ...
            // 平台费 -- 销售总额 * 平台费配置比例
            JsonAmount platformFeeJson = BeanUtil.copyProperties(saleContractItem.getUnitPrice(),JsonAmount.class);
            ...
            // 表单保险费
            JsonAmount insurance = Optional.ofNullable(saleContract.getInsuranceFee()).orElse(new JsonAmount().setAmount(BigDecimal.ZERO).setCurrency(CalculationDict.CURRENCY_RMB));
            ...
            if(!Objects.equals(saleContract.getTotalGoodsValue().getAmount(), BigDecimal.ZERO)){
                insuranceFee = NumUtil.mul(NumUtil.div(insurance.getAmount(), saleContract.getTotalGoodsValue().getAmount()), totalSaleAmount).setScale(CalculationDict.DECIMAL_PRECISION, RoundingMode.HALF_UP);
            }
            ...
            Optional.ofNullable(saleContractItem.getSinosureFee()).ifPresent(s->{
                ...
                Optional.ofNullable(sinosureFeeJson.getAmount()).ifPresent(x->{
                    ...
                    BigDecimal sinosureFee = NumUtil.div(NumUtil.mul(NumUtil.mul(creditInsuranceRate, totalSaleAmount), currencyRateMap.get(currency)), currencyRateMap.get(sinosureFeeJson.getCurrency()));
     ...
     * @return 转换结果
     */
    public static Map<String, BigDecimal> convertConfigValue(Map<String, String> configMap) {
     ...
     */
    public static void checkCommissionFee(SaleContractItem saleContractItem,String currency) {
        // 外销总金额
     ...
     */
    private static JsonWeight checkTotalGrossWeight(List<SaleContractItem> saleContractItemList, JsonWeight totalGroussweight) {
        if (CollUtil.isEmpty(saleContractItemList)) {
            return null;
        }
     ...
     */
    private static JsonWeight checkTotalWeight(List<SaleContractItem> saleContractItemList, JsonWeight totalWeight) {
        if (CollUtil.isEmpty(saleContractItemList)) {
            return null;
        }
     ...
     */
    private static BigDecimal chackTotalVolume(List<SaleContractItem> saleContractItemList, BigDecimal totalVolume) {
        if (CollUtil.isEmpty(saleContractItemList)) {
            return null;
        }
     ...
     */
    public static JsonAmount checkTotalGoodsValue(List<SaleContractItem> saleContractItemList, JsonAmount totalGoodsValue,String currency) {
        if (CollUtil.isEmpty(saleContractItemList)) {
            return null;
        }
     ...
     */
//    private static void checkTotalPurchase(List<SaleContractSaveReqVO> saleContractItemList, List<JsonAmount> totalPurchase, Map<String, BigDecimal> currencyRateMap) {
//        if (CollUtil.isEmpty(saleContractItemList)) {
//            return;
//        }
...
//    }
     ...
     */
    private static JsonAmount checkAccessoriesPurchaseTotal(List<SaleContractItem> saleContractItemList, JsonAmount accessoriesPurchaseTotal, Map<String, BigDecimal> currencyRateMap,String currency) {
        if (CollUtil.isEmpty(saleContractItemList)) {
            return null;
        }
        BigDecimal totalPurchaseCalc = saleContractItemList.stream()
     ...
     */
    private static JsonAmount checkTotalVatRefund(List<SaleContractItem> saleContractItemList, JsonAmount totalVatRefund,String currency) {
        if (CollUtil.isEmpty(saleContractItemList)) {
            return null;
        }
     ...
     */
    private static JsonAmount checkPlatformFee(List<SaleContractItem> saleContractItemList, Map<String, BigDecimal> currencyRateMap, Map<String, BigDecimal> transformedConfigMap, JsonAmount platformFee,String currency) {
        // 平台费
...
//                    return NumUtil.mul(totalSaleAmount.getAmount(), currencyRate);
     ...
     */
    private static JsonAmount checkCommissionAmount(List<SaleContractItem> saleContractItemList, JsonAmount commissionAmount,String currency) {
