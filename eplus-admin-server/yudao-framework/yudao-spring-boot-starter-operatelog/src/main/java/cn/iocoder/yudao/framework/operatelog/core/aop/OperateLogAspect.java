...
public class OperateLogAspect {
    ...
    public static void addExt(String key, Object value) {
        ...
        Object o = EXTS.get().get(key);
        if (Objects.isNull(o)){
            EXTS.get().put(key,value);
        }else {
            String content = o + CommonDict.COMMA + value;
            EXTS.get().put(key,content);
        }
    }
    ...
    private static void fillModuleFields(OperateLog operateLogObj,
        ...
        if (operateLog != null) {
            ...
            operateLogObj.setType(operateLog.name());
        }
        if (StrUtil.isEmpty(operateLogObj.getName()) && operation != null) {
            operateLogObj.setName(operation.summary());
        }
        if (StrUtil.isEmpty(operateLogObj.getType())&&operation!=null){
            operateLogObj.setType(operation.description());
        }
        // content 和 exts 属性
        ...
        if (content.contains("%s")) {
            content = String.format(content, operation.summary());
        }
    ...
    private static String obtainMethodArgs(ProceedingJoinPoint joinPoint) {
        ...
        // 过滤前端传值但为null的数据
       argValues = Arrays.stream(argValues).filter(Objects::nonNull).toArray();
        // 拼接参数
        Map<String, Object> args = Maps.newHashMapWithExpectedSize(argValues.length);
        // 避免数组越界应以实际传参数组长度进行遍历
