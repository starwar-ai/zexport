...
public class AreaUtils {
    ...
    private AreaUtils() {
        ...
        areas = new HashMap<>();
        areas.put(Area.ID_GLOBAL, new Area(Area.ID_GLOBAL, "全球", 0, CommonDict.HYPHEN, null, new ArrayList<>()));
        // 从 csv 中加载数据
        List<CsvRow> rows = CsvUtil.getReader().read(ResourceUtil.getUtf8Reader("area.csv")).getRows();
        rows.remove(0); // 删除 header
        for (CsvRow row : rows) {
            // 创建 Area 对象
            Area area = new Area(Integer.valueOf(row.get(0)), row.get(1), Integer.valueOf(row.get(2)), row.get(4),null, new ArrayList<>());
            // 添加到 areas 中
     ...
     * 根据节点获取完整id列表
     * 例如说:
     * id 为尖草坪对应id 100004 时， [10,1000,100004]
     *
     * @param id 子节点id
     * @return 完整id列表
     */
    public static List<Integer> formatList(Integer id) {
        // 获得区域
        Area area = areas.get(id);
        if (area == null) {
            return null;
    ...
    public static List<String> getAreaName(){
        return areas.values().stream().map(Area::getName).toList();
    }
