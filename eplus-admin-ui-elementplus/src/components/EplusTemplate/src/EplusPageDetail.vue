<template>
  <el-container
    :class="props.scrollFlag ? 'page-detail-container' : 'page-detail-container-no-scroll'"
  >
    <el-main ref="pageMainref">
      <div
    ...
    <el-footer class="pageAction">
      <EplusEditButton
        :isShow="props.edit ? true : false"
        :permi="props.edit?.permi"
        @click="props.edit?.handler"
      ...
      <EplusCancelButton
        ...
        @click="handleCancel"
        :startUserId="tasks[0]?.processInstance?.startUserId"
        :auditable="props.auditable"
        key="cancel"
      />
      <EplusApproveButton
      ...
      <EplusSubmitButton
        ...
        @click="props.submit?.handler"
        :auditable="props.auditable"
        key="submit"
        :user="props.submit?.user || ''"
      />
...
defineOptions({ name: 'EplusDetailView' })
...
const getTaskList = async (processInstanceId: string) => {
  try {
    ...
    auditUserIds.value = tasks.value
      .filter((it) => {
        return it.result == 1 || it.result == 6
      })
      .map((it) => it.assigneeUser.id)
    let currTasks = tasks.value.filter((it) => {
      return (it.result == 1 || it.result == 6) && it.assigneeUser.id == userId
    })
...
const pageMainref = ref()

const setPageHeight = () => {
  nextTick(() => {
    setTimeout(() => {
      const pageTopHeight = pageTopRef.value?.clientHeight
      const pageHeight = pageMainref.value.$el.clientHeight - 61
...
defineExpose({ getPageHeight, bottomHeight, goBack })

onMounted(async () => {
  if (props.auditable?.processInstanceId) {
    await getTaskList(props.auditable?.processInstanceId)
  }
  setPageHeight()
})
...
.page-detail-container-no-scroll {
