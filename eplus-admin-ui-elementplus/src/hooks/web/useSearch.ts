...
export const useSearch = () => {
  ...
  const methods = {
     ...
     */
    setSearchProps: async (props: SearchProps = {}) => {
     ...
     * @param data 需要设置的数据
     */
    setSearchValues: async (data: Recordable) => {
      const search = await getSearch()
     ...
     */
    setSearchSchema: async (schemaProps: FormSetPropsType[]) => {
      const search = await getSearch()
     ...
     * @param index 在哪里新增
     */
    addSearchSchema: async (formSchema: FormSchema, index?: number) => {
      const search = await getSearch()
     ...
     * @param field 删除哪个数据
     */
    delSearchSchema: async (field: string) => {
      const search = await getSearch()
     ...
     * @returns form data
     */
    getSearchFormData: async <T = Recordable>(): Promise<T> => {
      const search = await getSearch()
