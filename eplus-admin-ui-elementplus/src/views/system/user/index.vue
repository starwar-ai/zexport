        ...
        >
          ...
          <el-form-item>
            ...
            <el-button
              ...
              :loading="exportBankLoading"
              v-hasPermi="['system:user:export']"
            >
              <Icon icon="ep:download" />银行信息模板导出
            </el-button>
          </el-form-item>
        </el-form>
      </ContentWrap>
...
const exportBankLoading = ref(false)
const handleBankExport = async () => {
  try {
    // 导出的二次确认
    await message.exportConfirm()
    // 发起导出
    exportBankLoading.value = true
    const data = await UserApi.importUserTemplate(queryParams)
    download.excel(data, '银行信息模板.xls')
  } catch {
  } finally {
    exportBankLoading.value = false
  }
}
/** 操作分发 */
