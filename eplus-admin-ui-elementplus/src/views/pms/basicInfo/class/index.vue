  ...
  <el-row>
    <ContentWrap class="!w100%">
      <el-button
        type="primary"
        @click="handleCreate"
        v-hasPermi="['pms:category:create']"
      >
        新增
      </el-button>
    </ContentWrap>
    ...
    >
      <ContentWrap>
        ...
        >
          ...
          <p class="mr40px">
            <span class="font-600">全简码：</span>
            {{ selectedNodeInfo?.profixCode || '-' }}
          </p>
          <p class="mr40px">
            <el-button
              link
              ...
              v-hasPermi="['pms:category:update']"
            >
              编辑
            </el-button>
          </p>
            ...
            <template #default="scope">
              <div class="flex items-center">
                <el-button
                  ...
                  v-hasPermi="['pms:category:delete']"
                >
                  删除
                </el-button>
                <el-button
...
const getInfo = async () => {
  if (selectedNode.value?.id && selectedNode.value?.id != 0) {
    selectedNodeInfo.value = await ClassApi.getClassInfo(selectedNode.value?.id)
  } else {
    selectedNodeInfo.value = {}
  }
}
const getList = async () => {
  let res = await ClassApi.getClassPage(queryParams)
  list.value = res?.list || []
