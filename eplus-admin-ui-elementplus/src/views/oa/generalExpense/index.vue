<template>
  ...
  >
    ...
    <template #create>
      <general-expense-form mode="create" />
    </template>
  </eplus-dialog>
  ...
  >
    <template #create="{ key }">
      <FeeShareFormCom
        ...
        :getApi="GeneralApi.getGeneralReimb"
        :updateApi="GeneralApi.updateGeneralReimb"
        :id="key"
        mode="create"
      />
    </template>
...
import GeneralExpenseForm from './GeneralExpenseForm.vue'
import GeneralReimbDetail from './GeneralReimbDetail.vue'
...
defineOptions({ name: 'GeneralExpense' })
  ...
  fields: [
    ...
    },
    {
      ...
      formatter: async (args: any[]) => {
        let list = await GeneralApi.getcompanySimpleList()
...
const eplusTableSchema: EplusTableSchema = {
  getListApi: async (ps) => {
    const res = await GeneralApi.getGeneralReimbPage({ reimbType: 2, ...ps })
    return {
      list: res.list,
      total: res.total
    }
  ...
  delListApi: async (id) => {
    await GeneralApi.deleteGeneralReimb(id)
  },
  exportListApi: async (ps) => {
    return await GeneralApi.exportGeneralReimb(ps)
  },
  columns: [
    {
      prop: 'code',
          ...
          return (
            <div class="flex items-center">
              <el-button
                ...
                hasPermi="['oa:general-reimb:query']"
              >
                详情
              </el-button>
              <eplus-dropdown
                ...
                otherItems={[
                  {
                    ...
                    user: row.reimbUser,
                    otherKey: 'generalReimbDelete',
                    label: '删除',
                    permi: 'oa:general-reimb:delete',
                    handler: async (row) => {
                  ...
                  },
                  {
                    ...
                    otherKey: 'generalReimbClose',
                    label: '作废',
                    permi: 'oa:general-reimb:close',
                    handler: async (row) => {
                      handleClose(row.id)
                    }
                  }
...
const handleClose = async (id) => {
  ...
  await GeneralApi.closeGeneral(data)
  message.success('作废成功')
  handleRefresh()
}

const exportFileName = checkPermi(['oa:general-reimb:export']) ? '一般费用管理.xlsx' : ''

// const handleSubmit = async (data) => {
//   // 提交请求
//   let res = await GeneralApi.submitGeneralReimb(data.id)
//   if (res) {
...
// const handleUpdateFeeShare = async (row) => {
//   feeShareAmount.value = row.totalAmountList[0]
//   FeeShareRef.value?.openCreate('费用归集', row.id)
// }
