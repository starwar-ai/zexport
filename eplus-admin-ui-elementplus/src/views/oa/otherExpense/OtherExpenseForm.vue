...
import GeneralExpenseFeeShareForm from '@/views/oa/generalExpense/components/GeneralExpenseFeeShareForm.vue'
...
import ApplyListCom from '@/views/oa/generalExpense/components/ApplyListCom.vue'
...
const { t } = useI18n()
defineOptions({ name: 'OtherExpenseForm' })
...
watch(
  () => [ExpenseListRef.value?.AmountList, LoanAppListComRef.value?.loanTotalList],
  ([list, loanTotalList]) => {
...
const saveForm = async () => {
  try {
    ...
    if (props.mode == 'create') {
      await OtherApi.createOtherReimb({ reimbType: 4, ...paramsResult, submitFlag: 0 })
      message.success(t('common.createSuccess'))
      emit('handleSuccess', 'success')
      close()
    } else {
      await OtherApi.updateOtherReimb({ reimbType: 4, ...paramsResult, submitFlag: 0 })
...
const submitForm = async () => {
  try {
    ...
    if (paramsResult.auxiliaryType === 1) {
      ...
      if (props.mode == 'create') {
        await OtherApi.createOtherReimb({ reimbType: 4, ...paramsResult, submitFlag: 1 })
        message.success('提交成功')
        close()
        emit('handleSuccess', 'success')
      } else {
        await OtherApi.updateOtherReimb({ reimbType: 4, ...paramsResult, submitFlag: 1 })
        message.success('提交成功')
        close()
        emit('handleSuccess', 'success')
      }
...
onMounted(async () => {
  if (props.mode === 'edit') {
    await getApplyFlag()
    let res = await OtherApi.getOtherReimb(props.id)
