...
const submitForm = async () => {
  ...
  for (var it of formData.value.columns) {
    if (['userName', 'deptName'].includes(it.nameType) && it.javaType != 'Long') {
      message.error(it.columnComment + '：resp返回名称类型不为空的时候，java类型必须为Long')
      return
    }
  }
  try {
