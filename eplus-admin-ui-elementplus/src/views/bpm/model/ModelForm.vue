<template>
  ...
  >
    ...
    >
      ...
      <el-form-item
        prop="transferFlag"
        label="审批人和提交人是同一个人时，自动转交给直属上级审批"
        label-width="auto"
        class="ml-13"
      >
        <el-switch
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      ...
      if (formData.value.category && typeof formData.value.category === 'string') {
        formData.value.category = Number(formData.value.category)
      }
    } finally {
...
// const handleChangeState = async (val) => {
//   console.log(val, 'val')
