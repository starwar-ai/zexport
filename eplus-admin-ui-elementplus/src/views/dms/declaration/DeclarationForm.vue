...
defineOptions({ name: 'DeclarationForm' })
...
const basicSchemas: EplusFormSchema[] = reactive([
  {
    field: 'invoiceCode',
    label: '发票号',
    component: <el-input placeholder="-" />
  },
  {
    field: 'invoiceDate',
    label: '发票日期',
...
const saveForm = async () => {
  try {
    ...
    await DeclarationApi.updateDeclaration(paramsResult)
...
onMounted(async () => {
  if (props.mode === 'edit') {
    loading.value = true
    let res = await DeclarationApi.getDeclarationInfo({ id: props.id })
