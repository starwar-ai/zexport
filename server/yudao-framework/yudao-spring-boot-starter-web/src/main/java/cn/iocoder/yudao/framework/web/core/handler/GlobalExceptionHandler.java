...
public class GlobalExceptionHandler {
    ...
    public GlobalExceptionHandler(String applicationName, ApiErrorLogFrameworkService apiErrorLogFrameworkService, boolean errorLogEnable) {
        this.applicationName = applicationName;
        this.apiErrorLogFrameworkService = apiErrorLogFrameworkService;
        this.errorLogEnable = errorLogEnable;
    }
    ...
    public CommonResult<?> accessDeniedExceptionHandler(HttpServletRequest req, AccessDeniedException ex) {
        ...
        return CommonResult.error(new ErrorCode(FORBIDDEN.getCode(), FORBIDDEN.getMsg() + ":" + req.getRequestURI()));
    }
     ...
     * 处理客户端主动断开连接异常
     *
     * 当客户端在服务器响应完成前断开连接时（如用户刷新页面、关闭标签页、请求超时等），
     * 服务器会抛出此异常。这是客户端行为导致的正常现象，不是服务器错误，无需记录错误日志。
     *
     * @author 波波
     */
    @ExceptionHandler(value = ClientAbortException.class)
    public CommonResult<?> clientAbortExceptionHandler(HttpServletRequest req, ClientAbortException ex) {
        log.debug("[clientAbortExceptionHandler][url({}) 客户端主动断开连接]", req.getRequestURL());
        // 客户端已断开连接，返回 null 即可，响应无法送达
        return null;
    }
    ...
    private void initExceptionLog(ApiErrorLog errorLog, HttpServletRequest request, Throwable e) {
        ...
        if (CollectionUtils.isEmpty(Arrays.asList(stackTraceElements))){
            e.printStackTrace();
        }
