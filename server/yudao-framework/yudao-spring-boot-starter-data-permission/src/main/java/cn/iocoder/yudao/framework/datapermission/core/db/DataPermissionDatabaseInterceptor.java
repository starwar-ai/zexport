...
public class DataPermissionDatabaseInterceptor extends JsqlParserSupport implements InnerInterceptor {
    ...
    private String formatSql(Configuration configuration, BoundSql boundSql) {
        try {
            ...
            // 打印SQL日志出现异常，记录异常并返回原始SQL
            logger.warn("格式化SQL日志异常", th);
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) {
        ...
        // 检查是否包含 foreach 循环的参数
        if (boundSql.getParameterMappings() != null) {
            for (ParameterMapping parameterMapping : boundSql.getParameterMappings()) {
                String property = parameterMapping.getProperty();
                if (property.startsWith("__frch_")) {
                    return; // 如果包含 foreach 循环的参数，直接返回，不进行数据权限处理
                }
            }
        }
        ...
        try {
            ...
            // 兼容处理包含 deleted IN 的情况
            if (sql.contains("deleted IN")) {
                sql = sql.replaceFirst("WHERE deleted=0", "WHERE 1=1");
            }
            
            String rSql = parserSingle(sql, null);
            mpBs.sql(rSql);
            
            // 打印最终执行的SQL
            BoundSql newBoundSql = new BoundSql(ms.getConfiguration(), rSql, boundSql.getParameterMappings(), boundSql.getParameterObject());
//            logger.info("执行SQL：" + formatSql(ms.getConfiguration(), newBoundSql));
        } finally {
            // 添加是否需要重写的缓存
    ...
    public void beforePrepare(StatementHandler sh, Connection connection, Integer transactionTimeout) {
        ...
        if (sct == SqlCommandType.UPDATE || sct == SqlCommandType.DELETE) {
            ...
            try {
                ...
                ContextHolder.init(rules);
                // 处理 SQL
                String sql = parserMulti(mpBs.sql(), null);
                mpBs.sql(sql);
                
                // 打印最终执行的SQL
                BoundSql newBoundSql = new BoundSql(ms.getConfiguration(), sql, mpSh.boundSql().getParameterMappings(), mpSh.boundSql().getParameterObject());
//                logger.info("执行SQL：" + formatSql(ms.getConfiguration(), newBoundSql));
            } finally {
                // 添加是否需要重写的缓存
                addMappedStatementCache(ms);
    ...
    protected void processPlainSelect(PlainSelect plainSelect) {
        ...
        if (CollectionUtils.isNotEmpty(selectItems)) {
            //获取规则列表
            List<DataPermissionRule> rules = ruleFactory.getDataPermissionRule(null);
            if (CollUtil.isNotEmpty(rules)) {
                //目前仅数据权限规则 直接取第一条即可
                DataPermissionRule dataPermissionRule = rules.get(0);
                if (Objects.nonNull(dataPermissionRule)) {
                    FromItem fromItem = plainSelect.getFromItem();
                    if (Objects.nonNull(fromItem)) {
                        //去除当前select对象中无权限字段
                        selectItems.removeIf(s -> dataPermissionRule.checkFieldPermission(fromItem.toString(), s.toString()));
                    }
                }
            }
