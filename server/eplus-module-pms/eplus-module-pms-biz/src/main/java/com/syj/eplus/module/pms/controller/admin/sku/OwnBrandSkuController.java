...
@Tag(name = "管理后台 - sku")
@RestController
@RequestMapping("/pms/own-brand")
@Validated
public class OwnBrandSkuController {
    ...
    @PostMapping("/create")
    @Operation(summary = "创建自营产品")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:create')")
    ...
    @PutMapping("/update")
    @Operation(summary = "更新自营产品")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:update')")
    ...
    @Operation(summary = "删除自营产品")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:delete')")
    ...
    @GetMapping("/detail")
    @Operation(summary = "获得自营产品sku")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:query')")
    ...
    @GetMapping("/page")
    @Operation(summary = "获得自营产品sku分页")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:query')")
    public CommonResult<PageResult<CskuRespVO>> getOwnBrandPage(@Valid SkuPageReqVO pageReqVO) {
        pageReqVO.setCustProFlag(BooleanEnum.NO.getValue()).setOwnBrandFlag(BooleanEnum.YES.getValue());
    ...
    @Operation(summary = "导出自营产品sku Excel")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:export')")
    @OperateLog(type = EXPORT)
    public void exportOwnBrandExcel(@Valid SkuPageReqVO pageReqVO, HttpServletResponse response) throws IOException {
        pageReqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        List<CskuRespVO> list = skuService.getCskuPage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "自营产品.xls", "数据", CskuRespVO.class,
                list);
    }


    @PutMapping("/approve")
    @Operation(summary = "通过自营产品任务")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:audit')")
    public CommonResult<Boolean> approveOwnBrandTask(@Valid @RequestBody BpmTaskApproveReqDTO reqVO) {
        skuService.approveOwnBrandTask(WebFrameworkUtils.getLoginUserId(), reqVO);
        return success(true);
    }

    @PutMapping("/reject")
    @Operation(summary = "不通过自营产品任务")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:audit')")
    public CommonResult<Boolean> rejectOwnBrandTask(@Valid @RequestBody BpmTaskRejectReqDTO reqVO) {
        skuService.rejectOwnBrandTask(WebFrameworkUtils.getLoginUserId(), reqVO);
        return success(true);
    }

    @PutMapping("/submit")
    @Operation(summary = "提交自营产品任务")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:submit')")
    public CommonResult<Boolean> submitOwnBrandTask(@RequestParam Long id) {
        skuService.submitOwnBrandTask(id, WebFrameworkUtils.getLoginUserId());
        return success(true);
    }


    @PutMapping("/change")
    @Operation(summary = "变更自营产品资料")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:change')")
    ...
    @Operation(summary = "获得变更自营产品资料分页")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-change:query')")
    public CommonResult<PageResult<CskuRespVO>> getOwnSkuChangePage(@Valid SkuPageReqVO pageReqVO) {
        pageReqVO.setCustProFlag(BooleanEnum.NO.getValue()).setOwnBrandFlag(BooleanEnum.YES.getValue());
    ...
    @Operation(summary = "更新变更自营产品资料")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-change:update')")
    ...
    @Operation(summary = "删除变更自营产品资料")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('pms:own-brand-change:delete')")
    ...
    @Operation(summary = "通过自营产品变更任务")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:audit')")
    public CommonResult<Boolean> approveChangeOwnBrandTask(@Valid @RequestBody BpmTaskApproveReqDTO reqVO) {
        skuService.approveChangeOwnBrandTask(WebFrameworkUtils.getLoginUserId(), reqVO);
        return success(true);
    }

    @PutMapping("/change-reject")
    @Operation(summary = "不通过自营产品变更任务")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:audit')")
    public CommonResult<Boolean> rejectChangeOwnBrandTask(@Valid @RequestBody BpmTaskRejectReqDTO reqVO) {
        skuService.rejectChangeOwnBrandTask(WebFrameworkUtils.getLoginUserId(), reqVO);
        return success(true);
    }

    @PutMapping("/change-submit")
    @Operation(summary = "提交自营产品变更任务")
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:submit')")
    public CommonResult<Boolean> submitChangeOwnBrandTask(@RequestParam Long skuId) {
        skuService.submitChangeOwnBrandTask(skuId, WebFrameworkUtils.getLoginUserId());
        return success(true);
    }
    ...
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:pricing')")
    ...
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:onshelfFlag')")
    ...
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:advantage')")
    ...
    @PreAuthorize("@ss.hasPermission('pms:own-brand-sku:anti-audit')")
