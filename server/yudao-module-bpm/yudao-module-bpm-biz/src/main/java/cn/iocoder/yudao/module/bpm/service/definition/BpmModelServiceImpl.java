...
public class BpmModelServiceImpl implements BpmModelService {
    ...
    public PageResult<BpmModelPageItemRespVO> getModelPage(BpmModelPageReqVO pageVO) {
        ...
        // 执行查询
        List<Model> models = modelQuery.modelWithoutTenantId()
    ...
    public List<SimpleModelRespVO> getSimpleModelList(BpmModelPageReqVO pageVO) {
        ModelQuery modelQuery = repositoryService.createModelQuery();
        if (StrUtil.isNotBlank(pageVO.getKey())) {
            modelQuery.modelKey(pageVO.getKey());
        }
        ...
        return models.stream().map(s -> new SimpleModelRespVO().setKey(s.getKey()).setName(s.getName())).toList();
    }

    @Override
    public String getBpmModelByModelKey(String modelKey) {
    ...
    public String createModel(@Valid BpmModelCreateReqVO createReqVO, String bpmnXml) {
        ...
        // 写入流程模型拓展表
        int transferFlag = Objects.isNull(createReqVO.getTransferFlag()) ? CalculationDict.ZERO : createReqVO.getTransferFlag();
        BpmModelExtDO bpmModelExtDO = bpmModelExtMapper.selectOne(BpmModelExtDO::getModelKey, modelKey);
        if (Objects.isNull(bpmModelExtDO)) {
            bpmModelExtMapper.insert(new BpmModelExtDO().setModelKey(modelKey).setTransferFlag(transferFlag));
        } else {
            bpmModelExtMapper.update(new BpmModelExtDO().setTransferFlag(transferFlag), new LambdaQueryWrapperX<BpmModelExtDO>().eq(BpmModelExtDO::getModelKey, modelKey));
        }
    ...
    public BpmModelRespVO getModel(String id) {
        ...
        BpmModelExtDO bpmModelExtDO = bpmModelExtMapper.selectOne(BpmModelExtDO::getModelKey, model.getKey());
        if (Objects.nonNull(bpmModelExtDO)) {
            modelRespVO.setTransferFlag(bpmModelExtDO.getTransferFlag());
        }
        return modelRespVO;
    }
    ...
    public void updateModel(@Valid BpmModelUpdateReqVO updateReqVO) {
        ...
        // 更新模型拓展表
        Integer transferFlag = updateReqVO.getTransferFlag();
        bpmModelExtMapper.update(new BpmModelExtDO().setTransferFlag(transferFlag), new LambdaQueryWrapperX<BpmModelExtDO>().eq(BpmModelExtDO::getModelKey, model.getKey()));
    }
    ...
    public void deployModel(String id) {
        ...
        String oldDefinitionId = updateProcessDefinitionSuspended(model.getDeploymentId());
        ...
        // 2.5 更新历史流程definitionKey
        bpmProcessInstanceExtMapper.update(new BpmProcessInstanceExtDO().setProcessDefinitionId(definition.getId()),new LambdaQueryWrapperX<BpmProcessInstanceExtDO>().eq(BpmProcessInstanceExtDO::getProcessDefinitionId,oldDefinitionId));
    }
     ...
     */
    private String updateProcessDefinitionSuspended(String deploymentId) {
