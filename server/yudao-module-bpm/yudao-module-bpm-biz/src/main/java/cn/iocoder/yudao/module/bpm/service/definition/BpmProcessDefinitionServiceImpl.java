...
public class BpmProcessDefinitionServiceImpl implements BpmProcessDefinitionService {
    ...
    @Override
    public BpmProcessDefinitionRespVO getProcessDefinitionBpmnXMLByKey(String key) {
        if (StrUtil.isEmpty(key)) {
            throw exception(PROCESS_INSTANCE_KEY_EXISTS);
        }
        ProcessDefinitionQuery definitionQuery = repositoryService.createProcessDefinitionQuery();
        definitionQuery.processDefinitionKey(key);
        // 执行查询
        ProcessDefinition processDefinition = definitionQuery.latestVersion().singleResult();
        if (Objects.isNull(processDefinition)) {
            throw exception(PROCESS_DEFINITION_NOT_EXISTS);
        }
        Deployment deployment = getDeployment(processDefinition.getDeploymentId());
        BpmProcessDefinitionExtDO bpmProcessDefinitionExtDO = processDefinitionMapper.selectByProcessDefinitionId(processDefinition.getId());
        BpmFormDO form = formService.getForm(bpmProcessDefinitionExtDO.getFormId());
        return BpmProcessDefinitionConvert.INSTANCE.convert(processDefinition, deployment, bpmProcessDefinitionExtDO, form);
    }
