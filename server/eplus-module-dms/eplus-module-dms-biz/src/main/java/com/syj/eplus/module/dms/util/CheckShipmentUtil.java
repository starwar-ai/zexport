...
import static com.syj.eplus.module.sms.enums.ErrorCodeConstants.CONTRACT_TOTAL_VAT_REFUND;
import static com.syj.eplus.module.sms.enums.ErrorCodeConstants.CONTRACT_TOTAL_VOLUME;
import static com.syj.eplus.module.sms.enums.ErrorCodeConstants.CONTRACT_ZERO_VOLUME;
import static com.syj.eplus.module.sms.enums.ErrorCodeConstants.CURRENCY_ITEM_BOX_COUNT;
 ...
 * @Date：2024/7/8 16:55
 */
public class CheckShipmentUtil {
    public static void checkItemCost(ShipmentSaveReqVO shipmentSaveReqVO,Map<String, BigDecimal> currencyRateMap) {
        List<ShipmentItem> shipmentItemList = shipmentSaveReqVO.getChildren();
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
        // 体积合计
        BigDecimal totalVolume = shipmentSaveReqVO.getTotalVolume();
        ...
        // 计算明细费用
        checkItemFee(shipmentItemList,currencyRateMap);
        // 货值合计
        calcGoodsValueTotalAmount( shipmentSaveReqVO.getTotalGoodsValue(),shipmentItemList);
        //报关合计
        calcDeclarationTotalAmount(shipmentSaveReqVO.getTotalDeclaration(),shipmentItemList);
        //采购合计
        //calcPurchaseTotalAmount(shipmentSaveReqVO.getTotalPurchase(),shipmentItemList);
        // 其他合计
        calcTotalQuantity(shipmentSaveReqVO,shipmentItemList);
    }
     ...
     */
    private static void checkTotalWeight(List<ShipmentItem> shipmentItemList, JsonWeight totalWeight) {
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
        BigDecimal totalWeightCalc = shipmentItemList.stream()
                .map(s -> {
        ...
        if (Objects.isNull(totalWeight) || !NumUtil.bigDecimalsEqual(totalWeightCalc, totalWeight.getWeight().setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP))) {
            throw exception(SHIPMENT_TOTAL_WEIGHT_EXCEPTION, totalWeightCalc);
        }
    }
     ...
     */
    private static void checkTotalGrossWeight(List<ShipmentItem> shipmentItemList, JsonWeight totalGroussweight) {
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
        BigDecimal totalGroussweightCalc = shipmentItemList.stream()
                .map(s -> {
        ...
        if (Objects.isNull(totalGroussweight) || !NumUtil.bigDecimalsEqual(totalGroussweightCalc, totalGroussweight.getWeight().setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP))) {
            throw exception(SHIPMENT_TOTAL_GROSSWEIGHT_EXCEPTION, totalGroussweightCalc);
        }
    }
     ...
     */
    private static void checkTotalBoxCount(List<ShipmentItem> shipmentItemList, Integer totalBoxes) {
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
     ...
     */
    private static void checkTotalQuantity(List<ShipmentItem> ShipmentItemList, Integer totalQuantity) {
        if (CollUtil.isEmpty(ShipmentItemList)) {
            return;
        }
     ...
     * @param ShipmentItemList 出运明细列表
     */
    private static void checkItemFee(List<ShipmentItem> ShipmentItemList,Map<String, BigDecimal> currencyRateMap) {
        ShipmentItemList.forEach(ShipmentItem -> {
            // 校验明细佣金金额
//            checkCommissionFee(ShipmentItem);
            //校验明细退税金额
            checkItemVatRefund(ShipmentItem.getTaxRefundRate(), ShipmentItem.getPurchaseWithTaxPrice(),ShipmentItem.getShippingQuantity(), ShipmentItem.getTaxRefundPrice(),currencyRateMap);
            // 校验箱数
            // checkBoxCount(ShipmentItem);
        });
    }
    ...
    private static void checkItemVatRefund(BigDecimal taxRefundRate, JsonAmount purchaseWithTaxPrice,Integer quantity, JsonAmount taxRefundPrice, Map<String, BigDecimal> currencyRateMap) {
        ...
        if (Objects.isNull(purchaseWithTaxPrice)|| Objects.isNull(purchaseWithTaxPrice.getAmount())|| purchaseWithTaxPrice.getAmount().compareTo(BigDecimal.ZERO) == 0) {
            return;
        }
     ...
     */
    private static BigDecimal checkBoxCount(ShipmentItem shipmentItem) {
        // 数量
    ...
    /**
     * 计算报关合计
     * @param totalDeclaration 前端传入的报关合计
     * @param items 出运明细
     */
    private static void calcDeclarationTotalAmount(List<JsonAmount> totalDeclaration,List<ShipmentItem> items){
        ...
        result.forEach((k,v)->{
            ...
            if (totalAmount.setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP).compareTo(baseAmount.setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP))!=0){
                throw exception(SHIPMENT_DECLARATION_TOTAL_EXCEPTION);
            }
        });
    }
    /**
     ...
     * @param items 出运明细
     */
    private static void calcGoodsValueTotalAmount(List<JsonAmount> totalGoodsValue,List<ShipmentItem> items){
        ...
        result.forEach((k,v)->{
            ...
            BigDecimal baseAmount = totalAmountMap.get(k);
            if (totalAmount.setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP).compareTo(baseAmount.setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP))!=0){
                throw exception(SHIPMENT_GOODS_TOTAL_EXCEPTION,totalAmount);
            }
        });
    }
     ...
     */
    private static void checkTotalTaxRefundPrice(List<ShipmentItem> shipmentItemList, JsonAmount totalTaxRefundPrice) {
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
     ...
     */
    private static void checkCommissionAmount(List<ShipmentItem> shipmentItemList, JsonAmount commissionAmount) {
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
     ...
     */
    private static void checkInsuranceFee(List<ShipmentItem> shipmentItemList, JsonAmount insuranceFee) {
        if (CollUtil.isEmpty(shipmentItemList)) {
            return;
        }
     ...
     * @param children 出运明细
     */
    private static void calcTotalQuantity(ShipmentSaveReqVO updateReqVO,List<ShipmentItem> children){
        // 数量合计
        Integer totalQuantity = updateReqVO.getTotalQuantity();
        children.stream().map(ShipmentItem::getShippingQuantity).reduce(Integer::sum).ifPresent(s->{
            if (!s.equals(totalQuantity)){
                throw exception(SHIPMENT_TOTAL_QUANTITY_EXCEPTION);
            }
        });
        // 箱数合计
        Integer totalBoxes = updateReqVO.getTotalBoxes();
        children.stream().map(ShipmentItem::getBoxCount).reduce(Integer::sum).ifPresent(s->{
            if (!s.equals(totalBoxes)){
                throw exception(SHIPMENT_TOTAL_BOX_QUANTITY_EXCEPTION);
            }
        });
        //体积合计
        BigDecimal totalVolume = updateReqVO.getTotalVolume();
        // 外箱体积 * 箱数
        ...
        if (totalVolume.setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP).compareTo(calcTotalVolume.setScale(CalculationDict.DECIMAL_CHECK,RoundingMode.HALF_UP))!=0){
            throw exception(SHIPMENT_TOTAL_VOLUME_EXCEPTION);
        }
        // 总毛重
        JsonWeight totalGrossweight = updateReqVO.getTotalGrossweight();
        checkTotalGrossWeight(children,totalGrossweight);
        // 总净重
        JsonWeight totalWeight = updateReqVO.getTotalWeight();
        checkTotalWeight(children,totalWeight);
    }
