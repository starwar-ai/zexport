...
import com.syj.eplus.module.oa.controller.admin.otherreimb.vo.OtherReimbFeeShareRespVO;
...
import com.syj.eplus.module.oa.convert.other.OtherReimbConvert;
 ...
 * @Description: 其他费用报销实现类
 */
@Service
public class OtherReimbServiceImpl implements OtherReimbService {

    protected final Logger logger = LoggerFactory.getLogger(getClass());
    public static final String PROCESS_DEFINITION_KEY = "oa_other_reimb";
    @Resource
    ...
    public Long createOtherReimb(OtherReimbSaveReqVO createReqVO) {
        ReimbDO reimbDO = OtherReimbConvert.INSTANCE.convertReimbDO(createReqVO);
        ...
        Long otherReimbId = reimbService.createReimb(reimbDO);
        List<ReimbRepayDetailSaveReqVO> batchInsertList = dealReimbRepayDetail(createReqVO, otherReimbId);
        if (CollUtil.isNotEmpty(batchInsertList)) {
            batchInsertList.forEach(s -> {
                s.setReimbId(otherReimbId);
                s.setRepaySourceType(RepaySourceTypeEnum.OTHER_REIMB.getType());
        ...
        if (SubmitFlagEnum.SUBMIT.getStatus().equals(createReqVO.getSubmitFlag())) {
            dealFeeShare(feeShareList, auxiliaryType, feeShareAmount, createReqVO.getCompanyId(), otherReimbId, reimbDO);
            submitTask(otherReimbId);
        }
        //更新费用申请状态
    ...
    private List<ReimbRepayDetailSaveReqVO> dealReimbRepayDetail(OtherReimbSaveReqVO createReqVO, Long reimbId) {
        ...
        // 计算当前报销总金额
        List<JsonAmount> amountList = createReqVO.getAmountList();
        if (Objects.isNull(amountList) || amountList.isEmpty()) {
            throw exception(TOTAL_AMOUNT_NULL);
        }
        final BigDecimal[] amount = {amountList.get(0).getAmount()};
        String totalCurrency = amountList.get(0).getCurrency();
        ...
        if (CollUtil.isNotEmpty(loanAppRespDTOList)) {
            loanAppRespDTOList.forEach(simpleLoanAppRespDTO -> {
                ...
                ReimbRepayDetailSaveReqVO reimbRepayDetailSaveReqVO = new ReimbRepayDetailSaveReqVO()
                        ...
                        .setReimbId(reimbId)
                        .setRepaySourceType(RepaySourceTypeEnum.OTHER_REIMB.getType())
                        .setRepayStatus(RepayStatus.NOT_REPAID.getStatus());
                // 当前报销总额大于等于借款单未还款金额
                if (amount[0].compareTo(outstandingAmount) > 0) {
                    // 实际报销金额为当前报销总额-当前借款单未还款金额
                    amount[0] = amount[0].subtract(outstandingAmount);
                    reimbRepayDetailSaveReqVO.setRepayAmount(new JsonAmount().setCurrency(currency).setAmount(outstandingAmount));
                } else {
                    reimbRepayDetailSaveReqVO.setRepayAmount(new JsonAmount().setCurrency(currency).setAmount(amount[0]));
                }
    ...
    private void dealFeeShare(List<FeeShareReqDTO> feeShareList, Integer auxiliaryType, JsonAmount feeShareAmount, Long companyId, Long otherReimbId, ReimbDO reimbDO) {
...
//                feeShare.setAmount(feeShareAmount);
            ...
            var feeShare = new OtherReimbFeeShareRespVO();
            Map<Long, SimpleCompanyDTO> simpleCompanyDTOMap = companyApi.getSimpleCompanyDTO();
            feeShare.setCompanyId(companyId);
            if (CollUtil.isNotEmpty(simpleCompanyDTOMap)) {
                SimpleCompanyDTO simpleCompanyDTO = simpleCompanyDTOMap.get(companyId);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int updateOtherReimb(OtherReimbSaveReqVO updateReqVO) {
        ReimbDO reimbDO = OtherReimbConvert.INSTANCE.convertReimbDO(updateReqVO);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int deleteOtherReimb(Long id) {
        // 删除
        ...
        // 删除费用归属
        reimbShareService.deleteReimbShareByReimbId(id,FeeShareSourceTypeEnum.OTHER_REIMBURSE);
        return result;
    }

    @Override
    public OtherReimbRespVO getOtherReimb(OtherReimbDetailReq req) {
        //将不同入口的参数转换为otherReimbId
        Long otherReimbId = Objects.nonNull(req.getProcessInstanceId()) ? bpmProcessInstanceApi.selectAuditAbleIdByProcessInstanceId(req.getProcessInstanceId()) : req.getOtherReimbId();
        if (Objects.isNull(otherReimbId)) {
            logger.error("[一般费用报销详情]未获取到一般费用报销id");
            return null;
        }
        ReimbRespVO reimbResp = reimbService.getReimbResp(otherReimbId);
        if (Objects.isNull(reimbResp)) {
            return null;
        }
        OtherReimbRespVO otherOtherReimbRespVO = OtherReimbConvert.INSTANCE.convertOtherReimbRespByReimbResp(reimbResp);
        List<ApplyRespVO> applyList = applyService.getApplyListByIdList(reimbResp.getApplyIdList());
        if (CollUtil.isNotEmpty(applyList)) {
            otherOtherReimbRespVO.setApplyList(applyList);
        }
        //查询费用归属信息
        List<FeeShareRespDTO> feeShare = feeShareApi.getFeeShareDTO(FeeShareSourceTypeEnum.OTHER_REIMBURSE, otherOtherReimbRespVO.getCode());
        otherOtherReimbRespVO.setFeeShare(feeShare);

        //未提交直接返回即可
        if (BpmProcessInstanceResultEnum.UNSUBMITTED.getResult().equals(reimbResp.getAuditStatus())) {
            return otherOtherReimbRespVO;
        }
        return otherOtherReimbRespVO;
    }

    @Override
    public PageResult<OtherReimbRespVO> getOtherReimbPage(ReimbPageReqVO pageReqVO) {
        PageResult<ReimbRespVO> otherReimbPage = reimbService.getReimbPage(pageReqVO, FeeShareSourceTypeEnum.OTHER_REIMBURSE.getValue());

        PageResult<OtherReimbRespVO> otherReimbRespVOPageResult = OtherReimbConvert.INSTANCE.convertOtherPageResult(otherReimbPage);

        List<OtherReimbRespVO> list = otherReimbRespVOPageResult.getList();
        if (CollUtil.isNotEmpty(list)) {
            List<String> codeList = list.stream().map(OtherReimbRespVO::getCode).distinct().toList();
            Map<String, List<FeeShareRespDTO>> feeShareMap = feeShareApi.getFeeShareByCodeList(FeeShareSourceTypeEnum.OTHER_REIMBURSE.getValue(), codeList);
        ...
        return otherReimbRespVOPageResult.setList(list);
    }
    ...
    public void updateApproveUserByInstanceId(Long reimbId,Long userId) {
        reimbService.updateApproveUserByInstanceId(reimbId,userId);
    }
}
