...
@Tag(name = "管理后台 - OA通用申请单")
@RestController
@RequestMapping("/oa/general-apply")
public class GeneralApplyController {

    @Resource
    private ApplyService applyService;

    @PostMapping("/create")
    @Operation(summary = "创建通用报销申请单")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:query')")
    public CommonResult<Long> createApply(@Valid @RequestBody ApplySaveReqVO createReqVO) {
        // 设置申请单类型为通用类型
        createReqVO.setApplyType(ReimbTypeEnum.GENERAL.getValue());
        return success(applyService.createApply(createReqVO,FeeShareSourceTypeEnum.GENERAL_APPLY));
    }

    @PutMapping("/update")
    @Operation(summary = "更新通用报销申请单")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:update')")
    public CommonResult<Boolean> updateApply(@Valid @RequestBody ApplySaveReqVO updateReqVO) {
        // 设置申请单类型为通用类型
        updateReqVO.setApplyType(ReimbTypeEnum.GENERAL.getValue());
        applyService.updateApply(updateReqVO,FeeShareSourceTypeEnum.GENERAL_APPLY);
        return success(true);
    }

    @DeleteMapping("/delete")
    @Operation(summary = "删除通用报销申请单")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('oa:general-apply:delete')")
    ...
    @GetMapping("/detail")
    @Operation(summary = "获得通用报销申请单")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:query')")
    public CommonResult<ApplyRespVO> getApply(@RequestParam("id") Long id) {
            ApplyRespVO apply = applyService.getApply(new ApplyDetailReq().setApplyId(id), FeeShareSourceTypeEnum.GENERAL_APPLY);
            return success(apply);
    }
    @GetMapping("/audit-detail")
    @Operation(summary = "获得通用报销申请单详情")
    @Parameter(name = "id", description = "编号", required = true, example = "uuid")
    public CommonResult  <ApplyRespVO> getApplyByProcessId(@RequestParam("id")
        String id) {
        ApplyRespVO apply = applyService.getApply(new ApplyDetailReq().setProcessInstanceId(id),FeeShareSourceTypeEnum.GENERAL_APPLY);
        return success(apply);
    }
    @GetMapping("/page")
    @Operation(summary = "获得通用报销申请单分页")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:query')")
    public CommonResult<PageResult<ApplyRespVO>> getApplyPage(@Valid ApplyPageReqVO pageReqVO) {
        // 设置申请单类型为通用类型进行过滤
        pageReqVO.setApplyType(ReimbTypeEnum.GENERAL.getValue());
        return success( applyService.getApplyPage(pageReqVO));
    }

    @GetMapping("/export-excel")
    @Operation(summary = "导出通用报销申请单 Excel")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:export')")
    @OperateLog(type = EXPORT)
    public void exportApplyExcel(@Valid ApplyPageReqVO pageReqVO,
        ...
        // 导出 Excel
        ExcelUtils.write(response, "通用报销申请单.xls", "数据", ApplyRespVO.class,list);
    }

    @PutMapping("/approve")
    @Operation(summary = "通过任务")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:audit')")
    ...
    @PreAuthorize("@ss.hasPermission('oa:general-apply:submit')")
    public CommonResult<Boolean> submitTask(@RequestParam Long paymentId) {
        applyService.submitTask(paymentId, WebFrameworkUtils.getLoginUserId(),ReimbTypeEnum.GENERAL.getValue());
        return success(true);
    }

    @PutMapping("/close")
    @Operation(summary = "作废")
    @PreAuthorize("@ss.hasPermission('oa:general-apply:close')")
